...
class Chart extends Component {
  ...
  render() {
    ...
    var { students, scores, lop, namhoc, hocky } = this.state;
    var item = sessionStorage.getItem("item");
    for (let i = 0; i < scores.length; i++) {
      if (scores[i].tbhk < 2.0 && scores[i].lop === item) {
    ...
    for (let i = 0; i < scores.length; i++) {
      if (scores[i].status === "Cảnh báo học tập") {
        data_bar_chart[0].Số_SV += 1;
      }
      if (scores[i].status === "Đình chỉ học tập") {
        data_bar_chart[1].Số_SV += 1;
      }
      if (scores[i].status === "Thiếu tín chỉ") {
        data_bar_chart[2].Số_SV += 1;
      }
      if (scores[i].status === "Thiếu học phí") {
        data_bar_chart[3].Số_SV += 1;
      }
      if (scores[i].status === "Không") {
        data_bar_chart[4].Số_SV += 1;
      }
      if (scores[i].status === "Khen thưởng") {
    ...
    return (
      <Container>
        ...
        <br /> <br />
        <Chart2>
          <h3 style={{ textAlign: "center" }}>BIỂU ĐỒ THỐNG KÊ ĐIỂM</h3>
          <Center2>
        ...
        <Chart1>
          <h3 style={{ textAlign: "center" }}>BIỂU ĐỒ THỐNG KÊ TRẠNG THÁI</h3>
          <Center1>
