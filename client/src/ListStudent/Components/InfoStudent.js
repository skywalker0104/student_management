...
class InfoStudent extends Component {
  ...
  componentDidMount() {
    ...
    if (match) {
      ...
      var idScore = sessionStorage.getItem("idScore");
      CallApi(`studentscore/${idScore}`, "GET", null).then((res) => {
        var tempdata = res.data.ScoreDetail;
        console.log(tempdata.length);
        var data = res.data.ScoreDetail[tempdata.length - 1];
        this.setState({
          score: data,
        });
      });
      ...
      <Container className="container">
        ...
        <Site>
          ...
          <Gpa_site>
            ...
            <label>{score.sum_of_credits}/153</label> <br />
            <progress min="0" max="158" value={score.sum_of_credits}></progress>
            <p style={{ marginTop: "10px" }}>Điểm trung bình tích lũy:</p>
            <label>{score.gpa}</label>
            <p>Trạng thái: </p>
            <label
              className={
                score.status === "Khen thưởng" || score.status === "Không"
                  ? "change_status_green"
                  : "change_status_red"
              }
            >
